...
fn test_mcmc_basic_run() {
    ...
    assert!(population.individuals.len() > 0, "MCMC should generate posterior samples");
    
    // Check individuals have proper structure
    let first_sample = &population.individuals[0];
    assert!(first_sample.features.len() > 0, "Model should have features");
    ...
    println!("Train metrics - Acc: {:.4}, Sens: {:.4}, Spec: {:.4}", 
             train_acc, train_sens, train_spec);
    
    // Check Bayesian AUC is reasonable
    assert!(train_auc > 0.6, "Bayesian train AUC should be better than random: {}", train_auc);
    ...
    println!("Test metrics - Acc: {:.4}, Sens: {:.4}, Spec: {:.4}", 
             test_acc, test_sens, test_spec);
    
    assert!(test_auc > 0.6, "Bayesian test AUC should be better than random: {}", test_auc);
...
fn test_mcmc_with_sbs() {
    ...
    println!("Total MCMC samples after SBS: {}", population.individuals.len());
    ...
    assert!(n_features >= param.mcmc.nmin as usize, "Should have at least nmin features");
    ...
    println!("After SBS - Train AUC: {:.4}, Test AUC: {:.4}", train_auc, test_auc);
    
    assert!(train_auc > 0.6, "SBS models should have train AUC better than random: {}", train_auc);
    assert!(test_auc > 0.6, "SBS models should have test AUC better than random: {}", test_auc);
...
fn test_mcmc_different_lambda() {
    ...
    for lambda in lambdas {
        ...
        println!("Lambda {}: {} samples, Train AUC = {:.4}, Test AUC = {:.4}", 
                 lambda, population.individuals.len(), train_auc, test_auc);
        
        results.push((lambda, train_auc, test_auc));
        
        assert!(population.individuals.len() > 0);
        assert!(train_auc > 0.6 && train_auc <= 1.0, 
                "Lambda {} should give valid train AUC: {}", lambda, train_auc);
        assert!(test_auc > 0.6 && test_auc <= 1.0, 
                "Lambda {} should give valid test AUC: {}", lambda, test_auc);
    }
...
fn test_mcmc_different_burn_in() {
    ...
    for n_burn in burn_periods {
        ...
        println!("n_burn {}: {} samples, Train AUC = {:.4}, Test AUC = {:.4}", 
                 n_burn, population.individuals.len(), train_auc, test_auc);
        ...
        assert!(train_auc > 0.6 && train_auc <= 1.0, 
                "Burn-in {} should give valid train AUC: {}", n_burn, train_auc);
        assert!(test_auc > 0.6 && test_auc <= 1.0, 
                "Burn-in {} should give valid test AUC: {}", n_burn, test_auc);
    }
...
fn test_mcmc_different_data_types() {
    ...
    for data_type in data_types {
        ...
        println!("Data type '{}': {} samples, first AUC = {:.4}", 
                 data_type, population.individuals.len(), population.individuals[0].auc);
...
fn test_mcmc_different_iterations() {
    ...
    for n_iter in iterations {
        ...
        let population = &experiment.collections[0][0];
        println!("n_iter {}: {} samples generated", n_iter, population.individuals.len());
        ...
        assert!(population.individuals.len() >= expected_min, 
...
fn test_mcmc_reproducibility() {
    ...
    let (test_auc1, _, _, _) = 
        pop1.bayesian_compute_metrics(test_data1, train_threshold1);
    ...
    let (test_auc2, _, _, _) = 
        pop2.bayesian_compute_metrics(test_data2, train_threshold2);
    
    println!("Run 1: {} samples, Train AUC = {:.6}, Test AUC = {:.6}", 
             pop1.individuals.len(), train_auc1, test_auc1);
    println!("Run 2: {} samples, Train AUC = {:.6}, Test AUC = {:.6}", 
             pop2.individuals.len(), train_auc2, test_auc2);
    
    // Should have same number of samples
    assert_eq!(pop1.individuals.len(), pop2.individuals.len(), 
    ...
    println!("AUC differences - Train: {:.6}, Test: {:.6}", train_auc_diff, test_auc_diff);
    
    assert!(train_auc_diff < 0.01, "Same seed should produce similar train AUC: diff = {}", train_auc_diff);
    assert!(test_auc_diff < 0.01, "Same seed should produce similar test AUC: diff = {}", test_auc_diff);
    
    // First sample should be identical (features)
    assert_eq!(pop1.individuals[0].features, pop2.individuals[0].features, 
...
fn test_mcmc_different_seeds() {
    ...
    for seed in seeds {
        ...
        println!("Seed {}: {} samples, Train AUC = {:.4}, Test AUC = {:.4}", 
    ...
    assert!(train_aucs.iter().all(|&auc| auc > 0.6 && auc <= 1.0), 
            "All train AUCs should be better than random");
    assert!(test_aucs.iter().all(|&auc| auc > 0.6 && auc <= 1.0), 
...
fn test_mcmc_feature_selection() {
    ...
    for max_features in max_features_list {
        ...
        println!("Max features {}: using {} features, AUC = {:.4}", 
                 max_features, n_features, population.individuals[0].auc);
        
        assert!(n_features <= max_features * 2, "Should respect max_features limit");
    }
}
...
fn test_mcmc_serialization() {
    ...
    experiment.save_auto(&save_path).expect("Failed to save experiment");
    ...
    assert_eq!(experiment.collections[0][0].individuals.len(), loaded_exp.collections[0][0].individuals.len());
    ...
    assert_eq!(orig_pop.individuals[0].features, loaded_pop.individuals[0].features);
    
    println!("Original samples: {}, Loaded samples: {}", orig_pop.individuals.len(), loaded_pop.individuals.len());
...
fn test_mcmc_test_predictions() {
    ...
    // Check samples exist
    assert!(population.individuals.len() > 0, "Should have posterior samples");
    
    // Check features are present
    for (idx, individual) in population.individuals.iter().take(5).enumerate() {
        println!("Sample {} - features: {}, AUC: {:.4}", 
                 idx, individual.features.len(), individual.auc);
...
fn test_mcmc_experiment_display() {
    ...
    assert!(experiment.id.contains("mcmc"), "Experiment ID should mention MCMC");
    ...
    let population = &experiment.collections[0][0];
    println!("Number of posterior samples: {}", population.individuals.len());
}
...
fn test_mcmc_prevalence_epsilon() {
    ...
    for epsilon in epsilons {
        ...
        let population = &experiment.collections[0][0];
        println!("Epsilon {}: {} samples, first AUC = {:.4}", 
                 epsilon, population.individuals.len(), population.individuals[0].auc);
...
fn test_mcmc_sbs_different_nmin() {
    ...
    for nmin in nmin_values {
        ...
        println!("nmin {}: {} features kept, {} samples", 
                 nmin, n_features, population.individuals.len());
        
        assert!(n_features >= nmin as usize, "Should keep at least nmin features");
        assert!(population.individuals.len() > 0, "Should have posterior samples");
    }
}
...
fn test_mcmc_performance_metrics() {
    ...
    println!("Execution time: {:.2}s", experiment.execution_time);
    assert!(experiment.execution_time > 0.0, "Execution time should be recorded");
    ...
    assert!(population.individuals.len() > 100, "Should have reasonable number of samples");
}
