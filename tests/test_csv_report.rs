...
fn test_csv_report_has_correct_header() {
    ...
    assert!(lines.len() >= 3, "Expected at least 3 lines (header + best_model + fbm), got {}", lines.len());
    ...
    assert!(!header.contains("extra_params"), "extra_params should be split into individual columns");
...
fn test_csv_report_no_na_metrics_with_fit_auc() {
    let param = create_csv_test_params();
    assert!(matches!(param.general.fit, gpredomics::param::FitFunction::auc), "Test requires fit=auc");
    ...
    let metric_names = ["train_f1", "train_mcc", "train_ppv", "train_npv", "train_g_mean",
                        "test_f1", "test_mcc", "test_ppv", "test_npv", "test_g_mean"];
    for name in &metric_names {
        let idx = header_cols.iter().position(|&h| h == *name)
            .unwrap_or_else(|| panic!("Column {} not found in header", name));
        assert_ne!(best_cols[idx], "NA",
            "Column {} should not be NA when csv_report forces metric computation", name);
    }
    ...
    for name in &metric_names {
        ...
        assert_ne!(fbm_cols[idx], "NA",
            "FBM column {} should not be NA", name);
    }
...
fn test_csv_report_best_model_and_fbm_sections() {
    ...
    let sections: Vec<&str> = lines[1..].iter().map(|l| l.split(',').next().unwrap()).collect();
    assert!(sections.contains(&"best_model"), "Missing best_model section");
    assert!(sections.contains(&"fbm"), "Missing fbm section");
    // No jury since voting is disabled
    assert!(!sections.contains(&"jury"), "jury should not be present when voting is disabled");
...
fn test_csv_report_model_k_present() {
    ...
    let lines: Vec<&str> = content.lines().collect();
    let header_cols: Vec<&str> = lines[0].split(',').collect();
    let k_idx = header_cols.iter().position(|&h| h == "model_k")
    ...
    let fbm_cols: Vec<&str> = lines[2].split(',').collect();
    let k_avg: f64 = fbm_cols[k_idx].parse().expect("FBM model_k should be numeric");
...
fn test_csv_report_column_count_consistency() {
    ...
    for (i, line) in lines[1..].iter().enumerate() {
        ...
        assert_eq!(col_count, header_count,
            "Row {} has {} columns but header has {}", i + 1, col_count, header_count);
    }
...
fn test_csv_report_append_mode() {
    ...
    // Write once
    exp.export_csv_report(csv_path).expect("First CSV export failed");
    ...
    exp.export_csv_report(csv_path).expect("Second CSV export failed");
    ...
    assert_eq!(lines2, lines1 + data_rows_first,
        "Append mode should add {} data rows, got {} total (was {})", data_rows_first, lines2, lines1);
...
fn test_csv_report_with_voting_has_jury_section() {
    ...
    assert!(sections.contains(&"best_model"), "Missing best_model section");
    assert!(sections.contains(&"fbm"), "Missing fbm section");
    assert!(sections.contains(&"jury"), "Missing jury section when voting is enabled");

    // Jury row should also have computed metrics (no NA for f1/mcc/ppv/npv/g_mean)
    let jury_line = lines[1..].iter().find(|l| l.starts_with("jury")).expect("jury line not found");
    ...
    for name in &metric_names {
        ...
        assert_ne!(jury_cols[idx], "NA",
            "Jury column {} should not be NA", name);
    }

    // Check header column count matches jury row
    let header_count = header_cols.len();
    let jury_count = jury_cols.len();
    assert_eq!(jury_count, header_count,
        "Jury row has {} columns but header has {}", jury_count, header_count);
...
fn test_csv_report_parameter_columns() {
    ...
    let beam_method_idx = header_cols.iter().position(|&h| h == "beam_method").unwrap();
    assert_eq!(best_cols[beam_method_idx], "LimitedExhaustive");

    let beam_k_start_idx = header_cols.iter().position(|&h| h == "beam_k_start").unwrap();
    assert_eq!(best_cols[beam_k_start_idx], "1");

    let beam_k_stop_idx = header_cols.iter().position(|&h| h == "beam_k_stop").unwrap();
    assert_eq!(best_cols[beam_k_stop_idx], "3");

    // Check voting disabled
    let vote_idx = header_cols.iter().position(|&h| h == "vote_enabled").unwrap();
...
fn test_csv_report_metrics_are_valid_numbers() {
    ...
    let numeric_metrics = [
        "train_auc", "train_accuracy", "train_sensitivity", "train_specificity",
        "train_f1", "train_mcc", "train_ppv", "train_npv", "train_g_mean",
    ...
    for name in &numeric_metrics {
        let idx = header_cols.iter().position(|&h| h == *name)
            .unwrap_or_else(|| panic!("Column {} not found", name));
        let val: f64 = best_cols[idx].parse()
            .unwrap_or_else(|_| panic!("Column {} value '{}' is not a valid number", name, best_cols[idx]));
        assert!(val.is_finite(), "Column {} should be a finite number, got {}", name, val);
    }
