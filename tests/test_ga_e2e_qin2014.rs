...
fn test_ga_qin2014_basic_run() {
    ...
    assert!(!experiment.collections[0].is_empty(), "First collection should not be empty");
    ...
    assert!(best_model.k > 0, "Best model should use at least one feature");
    ...
    assert!(experiment.execution_time < 600.0, "Execution time should be less than 10 minutes");
...
fn test_ga_qin2014_with_keep_trace() {
    ...
    assert!(!experiment.collections.is_empty(), "Collections should not be empty");
    assert!(experiment.collections[0].len() > 1, "Should have multiple generations in trace");
    
    // Verify progression
    let first_gen = &experiment.collections[0][0];
    let last_gen = &experiment.collections[0][experiment.collections[0].len() - 1];
    
    assert!(!first_gen.individuals.is_empty(), "First generation should have individuals");
    assert!(!last_gen.individuals.is_empty(), "Last generation should have individuals");
    
    // Evolution should generally improve (not guaranteed, but likely with this dataset)
    println!("  - First gen best AUC: {:.4}", first_gen.individuals[0].auc);
...
fn test_ga_qin2014_multiple_data_types() {
    ...
    assert!(!data_types_found.is_empty(), "Should have at least one data type");
    println!("  - Data types found in final population: {:?}", data_types_found);
...
fn test_ga_qin2014_feature_selection() {
    ...
    assert!(experiment.train_data.feature_selection.len() > 0, "Should have selected features");
    assert!(experiment.train_data.feature_selection.len() <= experiment.train_data.feature_len,
...
fn test_ga_qin2014_model_complexity() {
    ...
    assert!(complex_k > 0, "Complex model should use at least one feature");
...
fn test_ga_qin2014_cv_enabled() {
    ...
    assert!(!experiment.final_population.as_ref().unwrap().individuals.is_empty(), "Final population must contain models");
...
fn test_ga_consistency_keep_trace() {
    ...
    assert!(exp_no_trace.collections[0].len() == 1, 
            "No trace should only keep final population");
    assert!(exp_with_trace.collections[0].len() > 1, 
            "With trace should keep all generations");
    
    println!("  - No trace: {} generation(s) stored", exp_no_trace.collections[0].len());
    println!("  - With trace: {} generations stored", exp_with_trace.collections[0].len());
...
fn test_ga_comprehensive_all_combinations() {
    ...
    for (lang_code, lang_name) in &languages {
        for (dtype_code, dtype_name) in &data_types {
            println!("\n┌─────────────────────────────────────────────────────────┐");
            println!("│ Testing: {} × {}                                    ", lang_name, dtype_name);
            println!("└─────────────────────────────────────────────────────────┘");
            ...
            for (backend_name, use_gpu, cv_name, use_inner_cv) in configs {
                ...
                param.general.save_exp = format!("test_comprehensive_{}_{}_{}_{}.mp", 
                    lang_code, dtype_code, backend_name, cv_name.replace(" ", "_"));
                ...
                assert!(experiment.final_population.is_some(), 
                    ...
                    lang_name, dtype_name, backend_name, cv_name);
                ...
                assert_eq!(best.language, expected_lang, 
                    "Language mismatch for {} {}", lang_name, dtype_name);
                assert_eq!(best.data_type, expected_dtype, 
                    "Data type mismatch for {} {}", lang_name, dtype_name);
                
                // Verify reasonable AUC
                assert!(best.auc >= 0.5 && best.auc <= 1.0, 
                    "Invalid AUC {:.3} for {} {} {} {}", 
                    best.auc, lang_name, dtype_name, backend_name, cv_name);
                
                println!("AUC={:.4}, k={}, pop_size={}", 
                    best.auc, best.k, final_pop.individuals.len());
                
                combination_results.push((backend_name, cv_name, best.auc, best.k, best.features.len()));
            ...
            for (backend, cv, auc, k, features) in &combination_results {
                println!("  │  • {} + {}: AUC={:.4}, k={}, features={}", 
                    backend, cv, auc, k, features);
            }
            
            // Check that all configurations produced valid results
            assert_eq!(combination_results.len(), 4, 
                "Should have 4 results for {} {}", lang_name, dtype_name);
            ...
            if auc_diff > 0.001 {
                println!("     ⚠ Warning: AUC difference = {:.6} (may indicate non-determinism)", auc_diff);
            } else {
