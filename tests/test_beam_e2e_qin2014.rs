...
fn test_beam_qin2014_basic_run() {
    ...
    assert!(Path::new(&param.data.X).exists(), "Training X file not found: {}", param.data.X);
    assert!(Path::new(&param.data.y).exists(), "Training y file not found: {}", param.data.y);
    assert!(Path::new(&param.data.Xtest).exists(), "Test X file not found: {}", param.data.Xtest);
    assert!(Path::new(&param.data.ytest).exists(), "Test y file not found: {}", param.data.ytest);
    ...
    println!("  DEBUG: collections.len() = {}", experiment.collections.len());
    if !experiment.collections.is_empty() {
        println!("  DEBUG: collections[0].len() = {}", experiment.collections[0].len());
        if !experiment.collections[0].is_empty() {
            println!("  DEBUG: collections[0][0].individuals.len() = {}", experiment.collections[0][0].individuals.len());
        }
    }
    ...
    assert!(!experiment.id.is_empty(), "Experiment ID should not be empty");
    assert!(!experiment.timestamp.is_empty(), "Timestamp should not be empty");
    ...
    assert!(experiment.train_data.sample_len > 0, "Train data should have samples");
    assert!(experiment.train_data.feature_len > 0, "Train data should have features");
    assert_eq!(experiment.train_data.classes.len(), 2, "Should have 2 classes");
    ...
    assert!(!experiment.collections.is_empty(), "Collections should not be empty");
    ...
    assert!(!beam_populations.is_empty(), "Beam populations should not be empty");
    
    // Beam with keep_trace should have multiple populations (one per k)
    println!("  - Number of k levels explored: {}", beam_populations.len());
    
    // Verify final population
    assert!(experiment.final_population.is_some(), "Final population should exist");
    let final_pop = experiment.final_population.as_ref().unwrap();
    assert!(!final_pop.individuals.is_empty(), "Final population should have individuals");
    
    // Verify best model
    let best_model = &final_pop.individuals[0];
    assert!(best_model.auc >= 0.0 && best_model.auc <= 1.0, "AUC should be between 0 and 1");
    assert!(best_model.sensitivity >= 0.0 && best_model.sensitivity <= 1.0, "Sensitivity should be between 0 and 1");
    assert!(best_model.specificity >= 0.0 && best_model.specificity <= 1.0, "Specificity should be between 0 and 1");
    assert!(best_model.k >= param.beam.kmin, "Best model should have at least kmin features");
    assert!(best_model.k <= param.beam.kmax, "Best model should have at most kmax features");
    assert!(!best_model.features.is_empty(), "Best model should have features");
    
    // Verify execution time is reasonable
    assert!(experiment.execution_time > 0.0, "Execution time should be positive");
    ...
    println!("  - Selected features: {}", experiment.train_data.feature_selection.len());
    ...
    println!("  - K range explored: [{}, {}]", param.beam.kmin, beam_populations.len() + param.beam.kmin - 1);
...
fn test_beam_qin2014_limited_exhaustive() {
    ...
    assert!(experiment.final_population.is_some(), "Should have final population");
    let final_pop = experiment.final_population.as_ref().unwrap();
...
fn test_beam_qin2014_serialization() {
    ...
    original_exp.save_auto(mp_file).expect("Failed to save experiment");
    ...
    assert_eq!(original_exp.gpredomics_version, loaded_exp.gpredomics_version);
    assert_eq!(original_exp.train_data.sample_len, loaded_exp.train_data.sample_len);
    assert_eq!(original_exp.train_data.feature_len, loaded_exp.train_data.feature_len);
    
    // Verify final population
    assert_eq!(
        original_exp.final_population.as_ref().unwrap().individuals.len(),
        loaded_exp.final_population.as_ref().unwrap().individuals.len()
    );
...
fn test_beam_qin2014_keep_trace() {
    ...
    assert!(!experiment.collections.is_empty(), "Collections should not be empty");
    let beam_populations = &experiment.collections[0];
    assert!(beam_populations.len() >= 1, "Should have at least one generation");
    
    println!("  - Number of k levels in trace: {}", beam_populations.len());
    ...
    for (i, pop) in beam_populations.iter().enumerate() {
        if !pop.individuals.is_empty() {
            ...
            println!("  - Level {}: k={}, n_models={}", i, k, pop.individuals.len());
        }
    }
...
fn test_beam_qin2014_early_stopping() {
    ...
    assert!(experiment.final_population.is_some(), "Should have final population");
    assert!(!experiment.final_population.as_ref().unwrap().individuals.is_empty(), 
...
fn test_beam_qin2014_best_models_ci_alpha() {
    ...
    assert!(exp_strict.final_population.is_some(), "Strict alpha should produce results");
    assert!(exp_relaxed.final_population.is_some(), "Relaxed alpha should produce results");
    
    let strict_size = exp_strict.final_population.as_ref().unwrap().individuals.len();
    let relaxed_size = exp_relaxed.final_population.as_ref().unwrap().individuals.len();
    
    println!("  - Strict alpha (0.99) final population: {} models", strict_size);
    println!("  - Relaxed alpha (0.001) final population: {} models", relaxed_size);
...
fn test_beam_qin2014_feature_selection_impact() {
    ...
    println!("  - Restrictive selection: {} features", restrictive_features);
    println!("  - Permissive selection: {} features", permissive_features);
    
    assert!(restrictive_features > 0, "Should have some features with restrictive selection");
    assert!(permissive_features >= restrictive_features, "Permissive should have more or equal features");
...
fn test_beam_qin2014_multiple_languages() {
    ...
    assert!(!languages_found.is_empty(), "Should have at least one language");
    println!("  - Languages found in final population: {:?}", languages_found);
...
fn test_beam_qin2014_reproducibility() {
    ...
    assert_eq!(best1.features, best2.features, "Features should be identical");
...
fn test_beam_qin2014_max_nb_models() {
    ...
    let final_size = experiment.final_population.as_ref().unwrap().individuals.len();
...
fn test_beam_qin2014_different_fit_functions() {
    ...
    for fit_fn in fit_functions {
        ...
        assert!(experiment.final_population.is_some(), "Should have final population for {:?}", fit_fn);
...
fn test_beam_qin2014_cv_enabled() {
    ...
    // CV Assertions
    assert!(experiment.cv_folds_ids.is_some(), "cv_folds_ids must be present in CV mode");
    let fold_ids = experiment.cv_folds_ids.as_ref().unwrap();
    assert_eq!(fold_ids.len(), param.cv.outer_folds, "cv_folds_ids length must equal outer_folds");

    assert_eq!(experiment.collections.len(), param.cv.outer_folds, "collections length must equal outer_folds");
    for (i, coll) in experiment.collections.iter().enumerate() {
        assert!(!coll.is_empty(), "Fold {} should have at least one population", i);
        let last_pop = &coll[coll.len() - 1];
        assert!(!last_pop.individuals.is_empty(), "Fold {} last population should have individuals", i);
        for ind in &last_pop.individuals {
            assert!(ind.k >= param.beam.kmin && ind.k <= param.beam.kmax,
                "Model k={} must be within [{}..{}]", ind.k, param.beam.kmin, param.beam.kmax);
        }
    }

    assert!(experiment.final_population.is_some(), "Final population should exist in CV mode");
    let final_pop = experiment.final_population.as_ref().unwrap();
    assert!(!final_pop.individuals.is_empty(), "Final population must contain models");
...
fn test_beam_qin2014_kmin_kmax_range() {
    ...
    for individual in &final_pop.individuals {
        assert!(individual.k >= param.beam.kmin, 
                "Model k={} should be >= kmin={}", individual.k, param.beam.kmin);
        assert!(individual.k <= param.beam.kmax, 
                "Model k={} should be <= kmax={}", individual.k, param.beam.kmax);
    }
    
    println!("  - kmin: {}", param.beam.kmin);
    println!("  - kmax: {}", param.beam.kmax);
    println!("  - All {} models respect k constraints", final_pop.individuals.len());
...
fn test_beam_qin2014_gpu_vs_cpu() {
    ...
    assert_eq!(best_cpu.auc, best_gpu.auc, 
...
fn test_beam_qin2014_all_language_datatype_combinations() {
    ...
    for language in &languages {
        for datatype in &datatypes {
            println!("\n--- Testing language={}, datatype={} ---", language, datatype);
            ...
            assert!(experiment.final_population.is_some(), 
                    "Should have final population for {}/{}", language, datatype);
            
            let final_pop = experiment.final_population.as_ref().unwrap();
            assert!(!final_pop.individuals.is_empty(), 
                    "Should have models for {}/{}", language, datatype);
            ...
            for (idx, individual) in final_pop.individuals.iter().enumerate() {
                ...
                if expected_lang == gpredomics::individual::TERNARY_LANG {
                    ...
                    if !has_neg && has_pos {
                        ...
                        assert_eq!(individual.language, expected_lang,
                ...
                assert_eq!(individual.data_type, expected_dtype,
                          "Individual {} should have data_type {} ({}), got {}", 
                          idx, datatype, expected_dtype, individual.data_type);
            }
            
            let best = &final_pop.individuals[0];
            println!("  ✓ {} models with language={}, datatype={}", 
                    final_pop.individuals.len(), language, datatype);
...
fn test_beam_gpu_with_inner_cv() {
    ...
    assert!(exp_cpu.final_population.is_some(), "CPU run should produce results");
    assert!(exp_gpu.final_population.is_some(), "GPU run should produce results");
    
    let pop_cpu = exp_cpu.final_population.as_ref().unwrap();
    let pop_gpu = exp_gpu.final_population.as_ref().unwrap();
    
    assert!(!pop_cpu.individuals.is_empty(), "CPU population should not be empty");
    assert!(!pop_gpu.individuals.is_empty(), "GPU population should not be empty");
    ...
    println!("Top GPU model: k={}, AUC={:.6}, features={:?}", 
    ...
    assert!(auc_diff < 1e-4,
        "AUC difference too large: CPU={:.6}, GPU={:.6}, diff={:.6}",
        best_cpu.auc, best_gpu.auc, auc_diff);
...
fn test_beam_consistency_inner_cv_vs_no_inner_cv() {
    ...
    let best_no_cv = &exp_no_inner_cv.final_population.as_ref().unwrap().individuals[0];
    let best_with_cv = &exp_with_inner_cv.final_population.as_ref().unwrap().individuals[0];
    
    println!("No Inner CV: AUC={:.4}, k={}", best_no_cv.auc, best_no_cv.k);
    println!("With Inner CV: AUC={:.4}, k={}", best_with_cv.auc, best_with_cv.k);
...
fn test_beam_consistency_outer_cv_vs_no_outer_cv() {
    ...
    let best_no_cv = &exp_no_outer_cv.final_population.as_ref().unwrap().individuals[0];
    let best_with_cv = &exp_with_outer_cv.final_population.as_ref().unwrap().individuals[0];
    
    println!("No Outer CV: AUC={:.4}, k={}", best_no_cv.auc, best_no_cv.k);
    println!("With Outer CV: AUC={:.4}, k={}", best_with_cv.auc, best_with_cv.k);
    
    // Verify CV structure
    assert!(exp_with_outer_cv.cv_folds_ids.is_some(), "CV should have fold IDs");
    assert_eq!(exp_with_outer_cv.collections.len(), param.cv.outer_folds, 
...
fn test_beam_consistency_gpu_vs_cpu_basic() {
    ...
    println!("CPU: AUC={:.6}, k={}, features={:?}", best_cpu.auc, best_cpu.k, best_cpu.features);
    println!("GPU: AUC={:.6}, k={}, features={:?}", best_gpu.auc, best_gpu.k, best_gpu.features);
...
fn test_beam_consistency_gpu_vs_cpu_inner_cv() {
    ...
    println!("CPU Inner CV: AUC={:.6}, k={}, features={:?}", 
             best_cpu.auc, best_cpu.k, best_cpu.features);
    println!("GPU Inner CV: AUC={:.6}, k={}, features={:?}", 
...
fn test_beam_consistency_keep_trace() {
    ...
    let best_with_trace = &exp_with_trace.final_population.as_ref().unwrap().individuals[0];
    
    println!("No trace: AUC={:.6}, k={}, features={:?}", 
             best_no_trace.auc, best_no_trace.k, best_no_trace.features);
    println!("With trace: AUC={:.6}, k={}, features={:?}", 
             best_with_trace.auc, best_with_trace.k, best_with_trace.features);
    
    // Beam is deterministic - final results should be identical
    assert_eq!(best_no_trace.features, best_with_trace.features, 
               "keep_trace should not affect final results");
    assert_eq!(best_no_trace.auc, best_with_trace.auc, 
    ...
    println!("  - No trace: {} k-level(s) stored", exp_no_trace.collections[0].len());
    println!("  - With trace: {} k-levels stored", exp_with_trace.collections[0].len());
    
    assert!(exp_with_trace.collections[0].len() >= exp_no_trace.collections[0].len(), 
...
fn test_beam_qin2014_with_voting() {
    ...
    assert!(experiment.others.is_some(), "Voting results should exist when voting is enabled");
    ...
    if let Some(gpredomics::experiment::ExperimentMetadata::Jury { jury }) = &experiment.others {
        ...
        assert!(jury.experts.individuals.len() > 0, "Should have at least one expert");
        assert!(jury.auc >= 0.0 && jury.auc <= 1.0, "Voting AUC should be valid");
        assert!(jury.sensitivity >= 0.0 && jury.sensitivity <= 1.0, "Voting sensitivity should be valid");
        assert!(jury.specificity >= 0.0 && jury.specificity <= 1.0, "Voting specificity should be valid");
    } else {
...
fn test_beam_qin2014_voting_methods() {
    ...
    for (method, method_name) in voting_methods {
        ...
        assert!(experiment.others.is_some(), "{} should produce voting results", method_name);
        
        if let Some(gpredomics::experiment::ExperimentMetadata::Jury { jury }) = &experiment.others {
            println!("  {} - Experts: {}, AUC: {:.4}", 
                     method_name, jury.experts.individuals.len(), jury.auc);
            
            assert!(jury.auc >= 0.0 && jury.auc <= 1.0, 
                    "{} should produce valid AUC", method_name);
        } else {
...
fn test_beam_qin2014_with_threshold_ci() {
    ...
    let best_model = &final_pop.individuals[0];
    assert!(best_model.threshold_ci.is_some(), "Best model should have threshold CI");
    ...
    assert!(threshold_ci.lower <= best_model.threshold, "CI lower should be <= threshold");
    assert!(threshold_ci.upper >= best_model.threshold, "CI upper should be >= threshold");
    assert!(threshold_ci.lower <= threshold_ci.upper, "CI lower should be <= upper");
    assert!(threshold_ci.rejection_rate >= 0.0 && threshold_ci.rejection_rate <= 1.0, 
...
fn test_beam_qin2014_threshold_ci_alpha_variations() {
    ...
    for &alpha in &alphas {
        ...
        let best_model = &experiment.final_population.as_ref().unwrap().individuals[0];
        assert!(best_model.threshold_ci.is_some(), "Should have threshold CI for alpha={}", alpha);
        
        let ci = best_model.threshold_ci.as_ref().unwrap();
        let ci_width = ci.upper - ci.lower;
        
        println!("  alpha={}: CI width = {:.4}, rejection_rate = {:.4}", 
                 alpha, ci_width, ci.rejection_rate);
        
        // CI width can be 0 if bootstrap produces exactly the same threshold (stable model)
        assert!(ci_width >= 0.0, "CI width should be non-negative for alpha={}", alpha);
        assert!(ci.lower <= best_model.threshold, "CI lower should be <= threshold");
        assert!(ci.upper >= best_model.threshold, "CI upper should be >= threshold");
    }
...
fn test_beam_qin2014_voting_with_threshold_ci() {
    ...
    if let Some(gpredomics::experiment::ExperimentMetadata::Jury { jury }) = &experiment.others {
        println!("  - Voting: {} experts, AUC={:.4}", 
                 jury.experts.individuals.len(), jury.auc);
        
        // Verify experts have threshold CI
        let experts_with_ci = jury.experts.individuals.iter()
            .filter(|ind| ind.threshold_ci.is_some())
            .count();
        
        println!("  - Experts with threshold CI: {}/{}", 
                 experts_with_ci, jury.experts.individuals.len());
        
        assert!(experts_with_ci > 0, "At least some experts should have threshold CI");
    } else {
        panic!("Expected Jury metadata");
    }
    ...
    let final_pop = experiment.final_population.as_ref().unwrap();
    let models_with_ci = final_pop.individuals.iter()
        .filter(|ind| ind.threshold_ci.is_some())
        .count();
    
    println!("  - Final models with threshold CI: {}/{}", 
             models_with_ci, final_pop.individuals.len());
    
    assert!(models_with_ci > 0, "At least some final models should have threshold CI");
...
fn test_beam_qin2014_voting_with_pruning() {
    ...
    if let Some(gpredomics::experiment::ExperimentMetadata::Jury { jury }) = &experiment.others {
        ...
        assert!(jury.experts.individuals.len() > 0, "Should have experts after pruning");
        ...
        for expert in &jury.experts.individuals {
            ...
            assert!(expert.auc >= 0.0 && expert.auc <= 1.0, "Expert should have valid AUC");
        }
    } else {
        panic!("Expected Jury metadata");
    }
