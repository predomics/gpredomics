...
pub fn compare_classes(values: &Vec<f64>, targets: &Vec<u8>, max_p_value: f64, min_prevalence: f64) -> u8 {
    ...
    // Calculate means
    let mean_0 = class_0.mean();
    let mean_1 = class_1.mean();
    ...
    let prev0 = class_0.iter().map(|x| if *x==0.0 {0.0} else {1.0}).sum::<f64>() / n0;
    let prev1 = class_1.iter().map(|x| if *x==0.0 {0.0} else {1.0}).sum::<f64>() / n1;
    ...
    let p_value = 2.0 * t_dist.cdf(-t_stat.abs()); // Two-tailed test
