...
mod tests {
    use super::*;
    use crate::individual::{Individual, RATIO_LANG, BINARY_LANG, TERNARY_LANG, POW2_LANG, RAW_TYPE, LOG_TYPE, PREVALENCE_TYPE};
    use crate::param::{GpuMemoryPolicy, GPU};
        ...
        for model in models {
            for sample_idx in 0..samples {
                let score = if model.language == RATIO_LANG {
                    ...
                    compute_additive_score_cpu(x_map, model, feature_selection, sample_idx, threshold)
                };
            ...
            if let Some(feature_pos) = feature_selection.iter().position(|&f| f == feature_idx) {
                if let Some(&x_value) = x_map.get(&(sample_idx, feature_pos)) {
                    let transformed_value = match model.data_type {
                        ...
                        PREVALENCE_TYPE => {
                            if x_value > threshold as f64 { 1.0 } else { 0.0 }
                        },
                        _ => x_value as f32,
                    };
    ...
    fn test_gpu_assay_creation_with_fallback() {
        ...
        assert!(result.is_ok(), "GPU assay creation should succeed with fallback");
    }
    ...
    fn test_gpu_memory_policy_adaptive() {
        ...
        assert!(result.is_ok(), "Adaptive policy should handle memory requests gracefully");
    ...
    fn test_gpu_memory_policy_strict_exceeds_limit() {
        ...
        let result = std::panic::catch_unwind(|| {
            GpuAssay::new(&x_map, &vec![0], 1, 1, &config)
        });
        
        // Either way, it should panic
        assert!(result.is_err(), "Strict policy with excessive memory should panic");
    }
    ...
    fn test_gpu_get_max_buffer_size_adaptive() {
        ...
        assert!(max_size <= 128 * 1024 * 1024, "Should respect requested limit or use hardware limit");
    }
    ...
    fn test_gpu_get_max_buffer_size_performance() {
        ...
        assert!(max_size > 1 * 1024 * 1024, "Performance mode should use more than requested");
    }
    ...
    fn test_gpu_compute_scores_basic() {
        ...
        assert!(scores.iter().all(|&s| s.is_finite()), "All scores should be finite");
        ...
        for i in 0..samples {
            ...
            assert!(diff < 1e-5, "Sample {} Model 1: expected {}, got {} (diff={})", 
                    i, expected_model1[i], scores[i], diff);
        }
        ...
        for i in 0..samples {
            ...
            assert!(diff < 1e-5, "Sample {} Model 2: expected {}, got {} (diff={})", 
                    i, expected_model2[i], scores[samples + i], diff);
        }
    }
    ...
    fn test_gpu_out_of_memory_handling() {
        ...
        assert!(result.is_ok(), "Adaptive should fallback to CPU when out of memory");
    }
    ...
    fn test_gpu_config_different_memory_policies() {
        ...
        for policy in policies {
            ...
            assert!(result.is_ok(), "Policy {:?} should work with fallback", policy);
        }
    }
    ...
    fn test_gpu_cpu_equivalence_binary_raw() {
        ...
        let cpu_scores = compute_scores_cpu_reference(&x_map, &models, &feature_selection, 
                                                       samples, 0.5);
        ...
        for i in 0..gpu_scores.len() {
            ...
            assert!(rel_error < 1e-3, 
                    ...
                    i, cpu_scores[i], gpu_scores[i], rel_error);
        }
    }
    ...
    fn test_gpu_cpu_equivalence_ternary_log() {
        ...
        let cpu_scores = compute_scores_cpu_reference(&x_map, &vec![model], &feature_selection, 
                                                       samples, 0.5);
        ...
        for i in 0..gpu_scores.len() {
            ...
            assert!(abs_error < 1e-4, 
                    ...
                    i, cpu_scores[i], gpu_scores[i], abs_error);
        }
    }
    ...
    fn test_gpu_cpu_equivalence_ratio_prevalence() {
        ...
        assert!(rel_error < 1e-3 || abs_error < 1.0, 
                "CPU={}, GPU={}, abs_error={}, rel_error={}", 
                cpu_scores[0], gpu_scores[0], abs_error, rel_error);
    }
    ...
    fn test_gpu_cpu_equivalence_comprehensive() {
        ...
        for i in 0..gpu_scores.len() {
            ...
            assert!(rel_error < 1e-3 || abs_error < tolerance, 
                    "Model {} Sample {}: CPU={}, GPU={}, rel_error={}, abs_error={}", 
                    model_idx, sample_idx, cpu_scores[i], gpu_scores[i], rel_error, abs_error);
        }
    }
    ...
    fn test_memory_policy_comparison() {
        ...
        let gpu_adaptive = GpuAssay::new(&x_map, &feature_selection, samples, max_model_nb, &config_adaptive);
        ...
        let gpu_perf = GpuAssay::new(&x_map, &feature_selection, samples, max_model_nb, &config_perf);
    ...
    fn test_wgsl_shader_compiles() {
        ...
        assert!(spgemm_source.contains("struct SpGemmParams"), "Missing SpGemmParams struct");
        assert!(spgemm_source.contains("@compute"), "Missing compute shader annotation");
    ...
    fn test_wgsl_shader_bindings() {
        ...
        for (binding, name) in expected_bindings {
            assert!(spgemm_source.contains(binding), "Missing binding: {}", binding);
        ...
        assert_eq!(binding_count, 9, "Expected 9 bindings, found {}", binding_count);
        
        let group_count = spgemm_source.matches("@group(0)").count();
        assert_eq!(group_count, 9, "Expected all bindings in @group(0), found {} groups", group_count);
    }
    ...
    fn test_wgsl_shader_workgroup_size() {
        ...
        assert!(spgemm_source.contains("@workgroup_size(16, 16)"), 
                "Expected workgroup_size(16, 16)");
        
        // Verify compute shader uses global_invocation_id
        assert!(spgemm_source.contains("@builtin(global_invocation_id)"),
    ...
    fn test_wgsl_shader_constants() {
        ...
        assert!(spgemm_source.contains("select("), "Missing select operation for prevalence");
        assert!(spgemm_source.contains("threshold"), "Missing threshold parameter");
        assert!(spgemm_source.contains("epsilon"), "Missing epsilon parameter");
        
        // Verify ratio computation with epsilon protection
        assert!(spgemm_source.contains("sumNeg"), "Missing sumNeg for ratio denominator");
        assert!(spgemm_source.contains("sumNeg + epsilon"), "Missing epsilon protection for division");
    }
    ...
    fn test_wgsl_shader_params_struct() {
        ...
        for field in required_fields {
            assert!(spgemm_source.contains(field), "Missing SpGemmParams field: {}", field);
        }
        
        // Verify params is declared as uniform
        assert!(spgemm_source.contains("var<uniform> params"),
    ...
    fn test_wgsl_shader_csr_csc_logic() {
        ...
        assert!(spgemm_source.contains("row_ptrX[row]"), "Missing CSR row pointer access");
        assert!(spgemm_source.contains("row_ptrX[row + 1u]"), "Missing CSR row+1 pointer access");
        assert!(spgemm_source.contains("col_idxX"), "Missing CSR column index array");
        
        // Verify CSC logic for MM matrix
        assert!(spgemm_source.contains("col_ptrMM[col]"), "Missing CSC column pointer access");
        assert!(spgemm_source.contains("col_ptrMM[col + 1u]"), "Missing CSC col+1 pointer access");
        assert!(spgemm_source.contains("row_idxMM"), "Missing CSC row index array");
        
        // Verify sparse matrix intersection logic
        assert!(spgemm_source.contains("if (fx < fmm)"), "Missing sparse intersection comparison");
        assert!(spgemm_source.contains("if (fx > fmm)"), "Missing sparse intersection comparison");
        assert!(spgemm_source.contains("else"), "Missing sparse intersection match case");
        
        // Verify loop structure
        assert!(spgemm_source.contains("loop {"), "Missing main computation loop");
        assert!(spgemm_source.contains("break"), "Missing loop break condition");
    }
    ...
    fn test_gpu_vs_individual_evaluate_all_combinations() {
        ...
        for (lang, lang_name) in &languages {
            for (dtype, dtype_name) in &data_types {
                ...
                // Validate results
                assert_eq!(gpu_scores.len(), cpu_scores.len(), 
                    "{} {}: Score count mismatch", lang_name, dtype_name);
                ...
                for (i, (&gpu_score, &cpu_score)) in gpu_scores.iter().zip(cpu_scores.iter()).enumerate() {
                    ...
                    assert!(diff < relative_tolerance, 
                        ...
                        lang_name, dtype_name, i, gpu_score, cpu_score, diff);
                }
                
                println!("✓ {} {} validated: {} samples", lang_name, dtype_name, samples);
            }
        }
