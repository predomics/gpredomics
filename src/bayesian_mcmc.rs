...
use crate::individual::{data_type, Individual, MCMC_GENERIC_LANG};
    ...
    pub fn update_feature_groups(
        ...
        for i_sample in 0..self.data.sample_len {
            if let Some(&raw) = self.data.X.get(&(i_sample, feature_idx)) {
                ...
                if delta_pos != 0.0 {
                    z_new[i_sample][0] += delta_pos * v;
                }
                if delta_neg != 0.0 {
                    z_new[i_sample][1] += delta_neg * v;
                }
            }
        }
...
impl MCMCAnalysisTrace {
    ...
    pub fn finalize(&mut self, n_iter: usize, n_burn: usize) {
        ...
        self.log_post_var =
            (self.log_post_var - self.log_post_mean.powf(2.0) * n_mean) / (n_mean - 1.0);
        ...
        for (&idx, &(p_pos, _p_neut, p_neg)) in &self.feature_prob {
            ...
            if p_pos > 0.0 {
                importances.push(Importance {
                    ...
                    aggreg_method: None,
                    importance: p_pos,
                    dispersion: (p_pos * (1.0 - p_pos) / self.population.individuals.len() as f64)
                        .sqrt(), // Bernoulli(P) repeated n_iter times
                    is_scaled: false,
                    scope_pct: 1.0,
                    direction: Some(1),
                });
            ...
            if p_neg > 0.0 {
                importances.push(Importance {
                    ...
                    aggreg_method: None,
                    importance: p_neg,
                    dispersion: (p_neg * (1.0 - p_neg) / self.population.individuals.len() as f64)
                        .sqrt(),
                    is_scaled: false,
                    scope_pct: 1.0,
                    direction: Some(0),
    ...
    for n in 0..param.mcmc.n_iter {
        if param.mcmc.n_iter > 1000
            && ((n as f64 == param.mcmc.n_iter as f64 * 0.25)
                || (n as f64 == param.mcmc.n_iter as f64 * 0.50)
                || (n as f64 == param.mcmc.n_iter as f64 * 0.75))
        {
            debug!(
                "MCMC : {}% iterations finished: a={:.4}, b={:.4}, c={:.4}",
                (n as f64 / param.mcmc.n_iter as f64) as f64 * 100.0,
                ind.get_betas()[0],
                ind.get_betas()[1],
                ind.get_betas()[2]
            );
...
pub fn mcmc(
    ...
    if param.mcmc.nmin != 0 && data.feature_selection.len() > param.mcmc.nmin as usize {
        ...
        for (nfeat, post_mean, _, log_evidence, feature_idx, _) in &results {
            cinfo!(
                param.general.display_colorful,
                "Features: {}, Posterior: {:.4e}, Log Evid: {:.4}, Removed: {}",
                nfeat,
                post_mean,
                log_evidence,
...
pub fn get_best_mcmc_sbs(
    ...
    let best_models = results
        .iter()
        .max_by(|(_, _, _, le1, _, _), (_, _, _, le2, _, _)| {
            le1.partial_cmp(le2).unwrap_or(std::cmp::Ordering::Equal)
        })
    ...
    for feature_idx in &features_to_drop {
        data_filtered
            .feature_selection
            .retain(|keep_idx| keep_idx != feature_idx);
    }

    info!(
        "\nBest Models: n = {}, data dimensions = ({}, {})",
        n_best,
        data_filtered.feature_selection.len(),
        data_filtered.sample_len
    );
