            ...
            while target_size>0 {
                ...
                sub_pop.generate(target_size, param.ga.kmin, 
                    ...
                    *data_type,
                    ...
                    data, param.general.threshold_ci_n_bootstrap > 0, rng);
...
pub fn iterative_evolution(base_pop: &Population, data: &mut Data, gpu_assay: &Option<GpuAssay>, param: &Param, running: Arc<AtomicBool>, rng: &mut ChaCha8Rng) -> Vec<Population> {
    ...
    loop {
        ...
        pop = evolve(pop, &epoch_data, &mut cv, param, &gpu_assays_per_fold, gpu_assay, epoch, &mut evolution_rng);
...
#[inline]
pub fn evolve(pop: Population, data: &Data, cv: &mut Option<CV>, param: &Param, gpu_assays_per_fold: &Vec<(Option<GpuAssay>, Option<GpuAssay>)>, gpu_assay: &Option<GpuAssay>, epoch: usize, rng: &mut ChaCha8Rng) -> Population {
...
fn select_parents(pop: &Population, param: &Param, rng: &mut ChaCha8Rng) -> Population {
    ...
    for i_type in sorted_keys {
        ...
        parents.individuals.extend(individuals_sorted.choose_multiple(rng, n2).map(|i| (*i).clone()));
    }
    parents
...
fn create_gpu_assays_for_folds(cv: &CV, param: &Param) -> Vec<(Option<GpuAssay>, Option<GpuAssay>)> {
...
mod tests {
    use super::*;
    use crate::individual::{BINARY_LANG, TERNARY_LANG, POW2_LANG, RATIO_LANG};
    ...
    fn test_select_parents_elite_selection() {
        ...
        assert!(parents.individuals.len() >= expected_min, 
            "Expected at least {} parents, got {}", expected_min, parents.individuals.len());
        ...
        assert_eq!(first_parent_fit, pop_best_fit, "Elite selection should pick best individuals");
    }
    ...
    fn test_select_parents_with_niche() {
        ...
        for i in 0..50 {
            ...
            ind.data_type = if i % 2 == 0 { PREVALENCE_TYPE } else { RAW_TYPE };
            ind.fit = (50 - i) as f64;
            ind.hash = i as u64;
        ...
        assert!(parents.individuals.len() > (50.0 * param.ga.select_elite_pct / 100.0) as usize,
    ...
    fn test_cross_over_creates_valid_children() {
        ...
        assert_eq!(children.individuals.len(), 20, "Should create requested number of children");
        ...
        for child in &children.individuals {
            ...
            assert_eq!(child.parents.as_ref().unwrap().len(), 2, "Child should have 2 parents");
            
            // Child k should match number of features
            assert_eq!(child.k, child.features.len(), "Child k should match feature count");
            
            // Child should inherit language and data_type from one parent
            assert_eq!(child.language, TERNARY_LANG, "Child should inherit parent language");
            assert_eq!(child.data_type, PREVALENCE_TYPE, "Child should inherit parent data_type");
        }
    }
    ...
    fn test_cross_over_with_different_languages() {
        ...
        assert_eq!(children.individuals.len(), 10, "Should create requested children");
        
        for child in &children.individuals {
            // Child should be valid
            assert!(!child.features.is_empty(), "Child should have features");
            assert!(child.language == TERNARY_LANG || child.language == BINARY_LANG,
    ...
    fn test_mutate_ternary_changes_features() {
        ...
        let original_features: Vec<_> = pop.individuals.iter()
        ...
        assert!(changed_count > 0, "At least some individuals should be mutated");
        assert!(changed_count <= pop.individuals.len(), "Not all should necessarily change");
    }
    ...
    fn test_mutate_ternary_respects_language() {
        ...
        for (_, &val) in &pop.individuals[0].features {
            assert!(val >= -1 && val <= 1, "Ternary values should be in range [-1, 1], got {}", val);
        }
    }
    ...
    fn test_mutate_binary_only_positive_values() {
        ...
        for ind in &pop.individuals {
            for (_, &val) in &ind.features {
                assert_eq!(val, 1, "Binary language should only have value 1, got {}", val);
            }
        }
    }
    ...
    fn test_mutate_pow2_power_values() {
        ...
        for ind in &pop.individuals {
            for (_, &val) in &ind.features {
                ...
                assert!(is_pow2 && abs_val <= 64, 
                    "POW2 value should be a power of 2 up to 64, got {}", val);
            }
        }
    }
    ...
    fn test_remove_stillborn_ternary() {
        ...
        assert_eq!(pop.individuals.len(), 1, "Should have 1 valid individual remaining");
        assert_eq!(pop.individuals[0].k, 2, "Valid individual should have 2 features");
    }
    ...
    fn test_mutate_with_zero_mutation_rate() {
        ...
        for i in 0..pop.individuals.len() {
            assert_eq!(pop.individuals[i].features, original_features[i],
    ...
    fn test_cross_over_preserves_feature_types() {
        ...
        for child in &children.individuals {
            ...
            for (_, &val) in &child.features {
                assert!(val >= -1 && val <= 1, 
                    "Ternary child should have values in [-1, 1], got {}", val);
            }
        }
    }
    ...
    fn test_mutate_updates_k_correctly() {
        ...
        for ind in &pop.individuals {
            assert_eq!(ind.k, ind.features.len(), 
                "k ({}) should match number of features ({})", ind.k, ind.features.len());
        }
    }
    ...
    fn test_select_parents_deterministic_with_seed() {
        ...
        assert_eq!(parents1.individuals.len(), parents2.individuals.len(),
            "Same seed should produce same number of parents");
        
        for i in 0..parents1.individuals.len() {
            assert_eq!(parents1.individuals[i].hash, parents2.individuals[i].hash,
                "Same seed should select same parents at index {}", i);
        }
    }
    ...
    fn test_cross_over_deterministic_with_seed() {
        ...
        for i in 0..children1.individuals.len() {
            assert_eq!(children1.individuals[i].features, children2.individuals[i].features,
                "Same seed should produce same child features at index {}", i);
            assert_eq!(children1.individuals[i].parents, children2.individuals[i].parents,
                "Same seed should produce same parent selection at index {}", i);
        }
    }
    ...
    fn test_evolve_produces_valid_population() {
        ...
        assert!(new_pop.individuals.len() > 0, "Evolve should produce individuals");
        ...
        for ind in &new_pop.individuals {
            ...
            assert!(ind.epoch > 0 || ind.k > 0, "Individual should be from current epoch or have features");
        }
    }
    ...
    fn test_mutate_preserves_language_and_data_type() {
        ...
        for i in 0..pop.individuals.len() {
            assert_eq!(pop.individuals[i].language, original_langs[i],
                "Mutation should not change language");
            assert_eq!(pop.individuals[i].data_type, original_dtypes[i],
    ...
    fn test_cross_over_requires_at_least_two_parents() {
        ...
        assert_eq!(children.individuals.len(), 5, "Should create requested children with 2+ parents");
        for child in &children.individuals {
            assert!(!child.features.is_empty(), "Child should have features");
            assert!(child.parents.is_some(), "Child should have parent information");
        }
    }
    ...
    fn test_mutate_pow2_doubling_and_halving() {
        ...
        assert!(found_doubling || found_halving, 
    ...
    fn test_mutate_pow2_boundary_values() {
        ...
        for (_, &val) in &pop.individuals[0].features {
            assert!(val.abs() <= 64, "POW2 values should not exceed 64, got {}", val);
        }
    }
    ...
    fn test_select_parents_empty_population() {
        ...
        assert_eq!(parents.individuals.len(), 0, "Empty population should return empty parents");
    }
    ...
    fn test_select_parents_all_same_type() {
        ...
        assert!(parents.individuals.len() >= 5, "Should select at least elite percentage");
        assert!(parents.individuals.len() <= 15, "Should not select too many");
    }
    ...
    fn test_cross_over_merges_features_correctly() {
        ...
        assert!(has_from_p1, "Some children should inherit features from parent 1");
        assert!(has_from_p2, "Some children should inherit features from parent 2");
    }
    ...
    fn test_mutate_ratio_language() {
        ...
        for (_, &val) in &pop.individuals[0].features {
            assert!(val >= -1 && val <= 1, "Ratio language should have values in [-1, 1], got {}", val);
        }
    }
    ...
    fn test_cross_over_conversion_between_languages() {
        ...
        for child in &children.individuals {
            assert!(!child.features.is_empty(), "Child should have features");
            assert_eq!(child.k, child.features.len(), "k should match feature count");
            
            // Check values are valid for child's language
            if child.language == TERNARY_LANG {
                for (_, &val) in &child.features {
                    assert!(val >= -1 && val <= 1, "Ternary child should have valid values");
                }
    ...
    fn test_mutate_with_100_percent_rate() {
        ...
        assert!(changed_count >= 7, "With 100% mutation rate, most individuals should change (got {})", changed_count);
    }
    ...
    fn test_select_parents_respects_percentages() {
        ...
        assert!(parents.individuals.len() >= 10, 
            "Should select at least elite_pct individuals");
        
        // Elite should be the best individuals
        for i in 0..10 {
            assert_eq!(parents.individuals[i].hash, pop.individuals[i].hash,
                "First {} parents should be elite (best individuals)", i);
        }
    }
    ...
    fn test_remove_stillborn_preserves_valid_individuals() {
        ...
        for (i, hash) in original_hashes.iter().enumerate() {
            assert_eq!(pop.individuals[i].hash, *hash, 
                "Valid individual {} should be preserved", i);
        }
    }
    ...
    fn test_selection_percentages_bounds_and_rounding() {
        ...
        assert!(parents.individuals.len() <= small_pop.individuals.len(),
        ...
        for ind in &parents.individuals {
            assert!(hashes.insert(ind.hash), 
                "Found duplicate individual in parents (hash={})", ind.hash);
        }
        ...
        assert!(all_elite.individuals.len() <= small_pop.individuals.len(),
    ...
    fn test_crossover_conflicting_feature_signs() {
        ...
        for child in &children.individuals {
            ...
            if let Some(&val) = child.features.get(&0) {
                assert!(val == 1 || val == -1, 
                    "Feature 0 with conflicting signs should resolve to +1 or -1, got {}", val);
            }
            
            // All values must be valid for child's language
            for (_, &val) in &child.features {
                assert!(val >= -1 && val <= 1, "Ternary values must be in [-1, 1], got {}", val);
    ...
    fn test_mutation_respects_feature_selection_boundaries() {
        ...
        for ind in &pop.individuals {
            for feature_idx in ind.features.keys() {
                ...
                assert!(*feature_idx <= 2 || feature_selection.contains(feature_idx),
                    "Feature {} should be from original or feature_selection", feature_idx);
            }
        }
    }
    ...
    fn test_no_zero_values_in_features_hashmap() {
        ...
        for ind in &pop.individuals {
            for (feature, &val) in &ind.features {
                assert_ne!(val, 0, 
                    "Feature {} has value 0 which should not exist in HashMap", feature);
            }
            assert_eq!(ind.k, ind.features.len(), 
        ...
        for child in &children.individuals {
            for (feature, &val) in &child.features {
                assert_ne!(val, 0,
                    "Child feature {} has value 0 which should not exist", feature);
            }
            assert_eq!(child.k, child.features.len(),
    ...
    fn test_clone_removal_after_evolution_step() {
        ...
        for ind in &children.individuals {
            assert!(seen_hashes.insert(ind.hash),
                "Found duplicate hash {} after clone removal", ind.hash);
        }
        
        // If clones were found, verify they were actually removed
        if clone_count > 0 {
            assert!(children.individuals.len() < 50,
    ...
    fn test_children_have_parent_information_and_epoch() {
        ...
        for (i, child) in children.individuals.iter().enumerate() {
            assert!(child.parents.is_some(),
                "Child {} should have parent information", i);
            
            let parent_hashes = child.parents.as_ref().unwrap();
            assert_eq!(parent_hashes.len(), 2,
                "Child {} should have exactly 2 parent hashes", i);
            ...
            assert!(p1_exists && p2_exists,
                "Child {} parents should exist in original population", i);
        }
    }
    ...
    fn test_forced_diversity_filtering() {
        ...
        assert!(parents.individuals.len() < initial_size,
            "Diversity filtering should reduce population from {} to {}",
            initial_size, parents.individuals.len());
        ...
        assert!(parents.individuals.len() <= size_before_strict,
    ...
    fn test_niche_quotas_per_type() {
        ...
        let mut type_counts: std::collections::HashMap<(u8, u8), usize> = std::collections::HashMap::new();
        for ind in &parents.individuals {
            *type_counts.entry((ind.language, ind.data_type)).or_insert(0) += 1;
        }
        
        // Each type should have some representation with niche selection
        assert_eq!(type_counts.len(), 4, 
        ...
        assert_eq!(parents.individuals.len(), parents2.individuals.len(),
            "Same seed should produce same parent count");
        
        for i in 0..parents.individuals.len() {
            assert_eq!(parents.individuals[i].hash, parents2.individuals[i].hash,
    ...
    fn test_mutation_non_null_chance_impact() {
        ...
        let avg_k_zero: f64 = pop_zero.individuals.iter()
            .map(|i| i.k as f64).sum::<f64>() / pop_zero.individuals.len() as f64;
        ...
        let avg_k_hundred: f64 = pop_hundred.individuals.iter()
            .map(|i| i.k as f64).sum::<f64>() / pop_hundred.individuals.len() as f64;
        
        // 100% should generally result in more features than 0%
        assert!(avg_k_hundred >= avg_k_zero,
    ...
    fn test_extreme_selection_percentages() {
        ...
        assert_eq!(parents_zero.individuals.len(), 0, 
        ...
        assert_eq!(parents_hundred.individuals.len(), 100,
            "100% elite should select all individuals");
        
        // Verify they're in order (elite = best first)
        for i in 0..100 {
            assert_eq!(parents_hundred.individuals[i].hash, pop.individuals[i].hash,
        ...
        for child in &children.individuals {
            ...
            match child.language {
                TERNARY_LANG | RATIO_LANG => {
                    for (_, &val) in &child.features {
                        assert!(val >= -1 && val <= 1,
                            "Ternary/Ratio child should have values in [-1,1], got {}", val);
                    }
                },
                POW2_LANG => {
                    for (_, &val) in &child.features {
                        assert!(val.abs() <= 64,
                            "POW2 child should have |value| <= 64, got {}", val);
                        let abs_val = val.abs();
                        assert!(abs_val > 0 && (abs_val & (abs_val - 1)) == 0,
                            "POW2 child should have power-of-2 values, got {}", val);
                    }
                },
                BINARY_LANG => {
                    for (_, &val) in &child.features {
    ...
    fn test_mutate_pow2_boundaries_strict() {
        ...
        for _ in 0..50 {
            ...
            if let Some(&val) = ind.features.get(&0) {
                assert!(val.abs() <= 64,
                    "Value at boundary should not exceed 64, got {}", val);
            }
        }
        ...
        for _ in 0..50 {
            ...
            if let Some(&val) = ind.features.get(&1) {
                assert!(val.abs() >= 1,
                    "Halving should not go below 1, got {}", val);
            }
        }
        ...
        for _ in 0..50 {
            ...
            if let Some(&val) = ind.features.get(&2) {
                ...
                assert!(val.abs() <= 64 && (val.abs() & (val.abs() - 1)) == 0,
                    "POW2 value should be valid power of 2, got {}", val);
            }
        }
    }
    ...
    fn test_remove_stillborn_loop_until_viable() {
        ...
        assert_eq!(pop.individuals.len(), initial_count - 10,
    ...
    fn test_crossover_inherits_data_type_from_main_parent() {
        ...
        for child in &children.individuals {
            assert!(child.data_type == PREVALENCE_TYPE || child.data_type == RAW_TYPE,
                "Child data_type should be from a parent, got {}", child.data_type);
    ...
    fn test_mutate_pow2_deterministic_branches() {
        ...
        for (_, &val) in &ind.features {
            if val != 0 {
                assert!(val.abs() <= 64 && (val.abs() & (val.abs() - 1)) == 0,
                    "POW2 value should be power of 2 <= 64, got {}", val);
            }
        }
    }
    ...
    fn test_selection_determinism_with_equal_fits_extensive() {
        ...
        for seed in vec![42, 123, 999] {
            ...
            assert_eq!(parents1.individuals.len(), parents2.individuals.len(),
                "Seed {} should produce consistent parent count", seed);
    ...
    fn test_equal_fit_deterministic_tiebreak_by_hash() {
        ...
        for run in 1..results.len() {
            assert_eq!(results[0].individuals.len(), results[run].individuals.len(),
                "Run {} should have same parent count as run 0", run);
            
            for i in 0..results[0].individuals.len() {
                assert_eq!(results[0].individuals[i].hash, results[run].individuals[i].hash,
        ...
        for i in 0..elite_count {
            assert_eq!(parents1.individuals[i].hash, parents2.individuals[i].hash,
                "Elite parent {} should be same regardless of seed (hash tie-break)", i);
        }
    }
    ...
    fn test_mutation_with_empty_and_partial_feature_selection() {
        ...
        let empty_selection: Vec<usize> = vec![];
        let original_features: Vec<_> = pop_empty.individuals.iter()
            .map(|i| i.features.clone()).collect();
        
        mutate(&mut pop_empty, &param, &empty_selection, &mut rng);
        
        for i in 0..pop_empty.individuals.len() {
            assert_eq!(pop_empty.individuals[i].features, original_features[i],
                "Empty feature_selection should not modify any features");
            assert_eq!(pop_empty.individuals[i].k, 3,
    ...
    fn test_crossover_gene_conflict_resolution() {
        ...
        for (idx, child) in children.individuals.iter().enumerate() {
            ...
            if let Some(&val) = child.features.get(&5) {
                assert!(val == 1 || val == -1,
                    "Child {} feature 5 conflict should resolve to +1 or -1, got {}", idx, val);
            ...
            for (feature, &val) in &child.features {
                assert_ne!(val, 0,
                    "Child {} feature {} should not be 0 (must be absent instead)", idx, feature);
            }
            
            // All values must be valid for ternary language
            for (_, &val) in &child.features {
                assert!(val >= -1 && val <= 1,
                    "Child {} should have ternary values in [-1,1], got {}", idx, val);
            }
            
            assert_eq!(child.k, child.features.len(),
                "Child {} k={} must equal features.len()={}", idx, child.k, child.features.len());
        }
    }
    ...
    fn test_forced_diversity_on_vs_off() {
        ...
        assert!(size_no_filter > 0, "Should select parents even with diversity off");
        ...
        assert!(parents_with_filter.individuals.len() < size_before_filter,
            "Forced diversity should reduce population from {} to {}",
            size_before_filter, parents_with_filter.individuals.len());
        
        // Verify remaining individuals are more diverse
        assert!(parents_with_filter.individuals.len() > 0,
        ...
        parents_strict = parents_strict.filter_by_signed_jaccard_dissimilarity(
            param.ga.forced_diversity_pct,
            true
        );
        
        // Strict filtering should reduce even more
        assert!(parents_strict.individuals.len() <= parents_with_filter.individuals.len(),
    ...
    fn test_unviable_params_detection() {
        ...
        assert_eq!(stillborn_count, initial_size as u32,
        ...
        assert_eq!(stillborn_binary, 20,
            "Binary individuals with k=0 should all be stillborn");
        assert_eq!(pop_binary.individuals.len(), 0,
    ...
    fn test_epoch_stamping_in_evolve() {
        ...
        let new_pop = evolve(pop, &data, &mut cv, &param, &vec![], &None, target_epoch, &mut rng);
    ...
    fn test_extreme_forced_diversity_single_parent() {
        ...
        assert!(parents.individuals.len() < initial_parent_count,
            "Extreme diversity should filter parents");
        
        let remaining_parents = parents.individuals.len();
        println!("Extreme diversity filtering left {} parent(s)", remaining_parents);
        ...
        if remaining_parents == 1 {
            ...
            println!("Note: cross_over requires at least 2 parents. With 1 parent, \
            ...
            assert_eq!(children.individuals.len(), 5,
                "Should create requested number of children");
            
            for (i, child) in children.individuals.iter().enumerate() {
                // Verify valid k
                assert_eq!(child.k, child.features.len(),
                    "Child {} must have k matching features.len()", i);
                ...
                let parent_hashes = child.parents.as_ref().unwrap();
                assert_eq!(parent_hashes.len(), 2,
                    "Child {} should have 2 parent hashes", i);
                
                // No zero values
                for (feature, &val) in &child.features {
                    assert_ne!(val, 0,
                        "Child {} feature {} should not be 0", i, feature);
                }
            }
        } else {
